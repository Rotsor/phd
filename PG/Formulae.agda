...
_≈f_ : ∀ B V → PGFormula B V → PGFormula B V → Set
a ≈f b = ∀ assign alg → pg-eval a assign alg ≈ pg-eval b assing alg

pg-eval : {A B G : Set} → (_+ˢ_ _⇾ˢ_ : G → G → G) → (εˢ : G) → (varˢ : A → G) → (condˢ : B → G → G) → PGFormula B A → G
