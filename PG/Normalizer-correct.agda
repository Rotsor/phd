...
module WithOrder {_<_ : A → A → Set} (A-STO : IsStrictTotalOrder _≡_ _<_) where
  ...
  +-preserves' : ∀ x y → fromNF x + fromNF y ≈ fromNF (x +' y)
  +-preserves' [] y = +-identityˡ -- (fromNF y)
  +-preserves' (x ∷ xs) y = +-assoc ⟨ trans ⟩ +-cong refl (+-preserves' xs y)
  ...
  ⇾ʳ-preserves x (y ∷ ys) = distribˡ ⟨ trans ⟩ +-cong (⇾₁-preserves x y) (⇾ʳ-preserves x ys) ⟨ trans ⟩ +-preserves' (x ⇾₁ y) (x ⇾ʳ ys)

  ⇾-preserves' : ∀ x y → fromNF x ⇾ fromNF y ≈ fromNF (x ⇾' y)
  ⇾-preserves' [] y = ⇾-identityˡ
  ⇾-preserves' (x ∷ xs) y = distribʳ ⟨ trans ⟩ ( +-cong (⇾ʳ-preserves x y) (⇾-preserves' xs y) ⟨ trans ⟩ +-preserves' (x ⇾ʳ y) (xs ⇾' y))

  +-preserves : ∀ x y → x + y ≈ fromNF (normalize x +' normalize y)
  ⇾-preserves : ∀ x y → x ⇾ y ≈ fromNF (normalize x ⇾' normalize y)
  normalize-preserves : ∀ f → f ≈ fromNF (normalize f)

  +-preserves x y = +-cong (normalize-preserves x) (normalize-preserves y) ⟨ trans ⟩ +-preserves' (normalize x) (normalize y) 
  ⇾-preserves x y = ⇾-cong (normalize-preserves x) (normalize-preserves y) ⟨ trans ⟩ ⇾-preserves' (normalize x) (normalize y) 
  ...
  normalize-preserves (x + y) = +-preserves x y
  normalize-preserves (x ⇾ y) = ⇾-preserves x y
  normalize-preserves ε = refl
  normalize-preserves (var a) =  sym true-condition ⟨ trans ⟩ sym +-identityʳ
  normalize-preserves ([ c ] y) = cond-cong BEq.refl (normalize-preserves y) ⟨ trans ⟩ mapConditions-correct c (normalize y)
